...
| `hardware-experiment.yml` dispatch run 16 on `6f4c803` (diagnostics-observability commit) | `6f4c803` | DISPATCHED to root-cause run 15: service now captures collector stdout/stderr into `collector_stdout.log`/`collector_stderr.log` inside the protected cell root, persists service failures to `service_diagnostic.log`, and the workflow uploads `hardware-diagnostics-<image>-<rep>` artifacts (`if: always()`) before cell-root cleanup. `operator_acknowledgement=APPROVE`. Run URL below. | https://github.com/infogyre/SnipeSpotter/actions/runs/34924414262 |
