...
Host is Linux; items below are native Windows CI (GitHub-hosted `windows-2025-vs2026` runners).

| Command | Commit SHA | Result | Evidence link |
| --- | --- | --- | --- |
| PR CI run 187: `ci.yml` full run (checks + elevated-windows) on `9763686` | `976368690fb9f5e9ecc111b17eaa003bb4b15e89` | SUCCESS | https://github.com/infogyre/SnipeSpotter/actions/runs/34807252442 |
| `cargo test --workspace --all-targets` (native Windows) — core 50, svc 155, cli 25 lib + 4 binary_contract + 2 scm_options, hardware path_policy 11, win32 11 incl. `dpapi_wipes_output_before_local_free`, dpapi_pipe 3 — all PASS | `36559ec5` (run 188) | PASS | https://github.com/infogyre/SnipeSpotter/actions/runs/34808178160/job/103864095156 |
| `cargo test -p spotter-svc --all-targets --features test-support --locked` — svc lib 156 (incl. all journal state-table tests, `ambiguous_tail_blocks_all_recovery_callers`, `ambiguous_tail_never_reissues_mutation`, `recovery_notice_is_bounded_and_redacted`, `unconfigured_settings_and_blocked_marker_stop_before_ipc`, `windows_acl::data_acl_contract`), atomic_interruption 13 (native ACL contracts), hosted_hardware 6 (native SMBIOS/WMI), live_status 2, native_pipe_concurrency 3 incl. `first_instance_claim_and_worker_lifetime` (AC.4), owner_fsm 24 incl. `token_owner_service_failure_table` — all PASS | `36559ec5` (run 188) | PASS | same job link |
| `cargo test -p spotter-cli --test binary_contract --features test-support --locked` — 10 tests PASS | `36559ec5` (run 188) | PASS | same job link |
| `cargo clippy --workspace --all-targets -- -D warnings` (native Windows) | `36559ec5` (run 188) | PASS | same job link |
| `cargo check -p spotter-hardware-service --features hardware-experiment --locked` (native Windows) | `36559ec5` (run 188) | PASS | same job link |
| PSScriptAnalyzer 1.25.0 — 21 PowerShell files clean (native Windows) | `36559ec5` (run 188) | PASS | same job link |
| Hardware workflow + privacy Python contracts, 36 tests (native Windows) | `36559ec5` (run 188) | PASS | same job link |
| Elevated Windows MSI lifecycle: MSI built from source on runner, install/verify/uninstall validated, direct-SCM test-support CLI lifecycle passed (`NT AUTHORITY\SYSTEM` observed); lifecycle logs artifact `elevated-windows-34808178160` | `36559ec5` (run 188) | PASS | https://github.com/infogyre/SnipeSpotter/actions/runs/34808178160/job/103864095174 |
| AC.2–AC.3 fixture tests in `spotter-cli/tests/named_pipe.rs` (`counterfeit_server_receives_no_request_bytes`, `server_identity_failures_reject_before_write`, `identity_fixture_actor_boundary`, `identity_fixture_failure_cleanup`, `server_exit_during_authentication_fails_closed`, `a0_probe_named_pipe_server_process_id`, `secured_server_and_production_client_roundtrip_on_unique_pipe`, `named_pipe_client_limits_impersonation_level`, `client_timeout_does_not_cancel_handler`) | previously feature-gated with no CI job running the target; wired into `checks.yml` Windows workspace job by PR #11 (`ee81ebd`) | RESOLVED — all pass natively (see following row) | https://github.com/infogyre/SnipeSpotter/actions/runs/34921511663 |
| `installed_service_authenticates_and_reconnects` (native fixture) | PR #11 merge `ee81ebd`, CI run 191 | **PASS — all 10 named_pipe fixtures executed natively** on the Windows workspace checks job (windows-2025 runner, head `c3a8fa2`/merge `0915c50`): `identity_fixture_actor_boundary`, `a0_probe_named_pipe_server_process_id`, `counterfeit_server_receives_no_request_bytes`, `identity_fixture_failure_cleanup`, `installed_service_authenticates_and_reconnects`, `named_pipe_client_limits_impersonation_level`, `server_exit_during_authentication_fails_closed`, `secured_server_and_production_client_roundtrip_on_unique_pipe`, `server_identity_failures_reject_before_write`, `client_timeout_does_not_cancel_handler` — all ok. Closes the AC.2–AC.4 native fixture evidence for SPOTR-5. | https://github.com/infogyre/SnipeSpotter/actions/runs/34921511663/job/104230488580 |
| `hardware-experiment.yml` dispatch run 15 on `36559ec` (6 cells: windows-2022/-latest × 3 reps; `operator_acknowledgement=APPROVE` honored by the workflow gate) | `36559ec5` | **FAILED — genuine native evidence**: interactive-admin collection + `validate_report.py` PASSED on all 6 cells; the LocalSystem service observation phase timed out (no LocalSystem report written; ~4 min gap per cell), then the 120 s deletion wait timed out with `DeleteService FAILED 1072` (service marked for deletion); cleanup **fail-closed exactly as designed** — cell root retained, root not deleted while service deletion unconfirmed. Lane-C defect candidate: LocalSystem observation never completes on hosted runners. Investigation required before any AC.8–AC.10 native pass evidence. | https://github.com/infogyre/SnipeSpotter/actions/runs/34853440794 |
| `hardware-experiment.yml` dispatch run 16 on `6f4c803` (diagnostics-observability commit) | `6f4c803` | FAILED but **observability achieved**: all 6 cells uploaded `hardware-diagnostics-<image>-<rep>` artifacts; `service_diagnostic.log` revealed the true run-15 cause — `unsafe hardware-host path ...config.json: path has a standard-user-writable ancestor`: the whole-chain ancestor write-ACE check rejected `C:\ProgramData`'s by-design standard-user create ACEs, so the service never started the collector. Fixed by `ebc4411` (gather ancestor facts only below the staging root). | https://github.com/infogyre/SnipeSpotter/actions/runs/34924414262 |
| `hardware-experiment.yml` dispatch run 17 on `ebc4411` | `ebc4411` | FAILED, one layer deeper: staging-root objects now validate; the allowlisted PowerShell host `C:\Program Files\PowerShell\7\pwsh.exe` was rejected by the same whole-chain ancestor check (OS-managed `Program Files` prefix carries by-design append/create ACEs). Fixed by `a0ea293` (ancestor facts gathered only below the staging root for all purposes; pwsh trusted via layout allowlist + no-follow walk + owner allowlist). | https://github.com/infogyre/SnipeSpotter/actions/runs/34925219718 |
| `hardware-experiment.yml` dispatch run 18 on `a0ea293` | `a0ea293` | FAILED, final-object layer: `pwsh.exe` itself carries by-design group write ACEs beyond SYSTEM/Administrators on managed systems (runner provisioning/installer ACEs). Fixed by `144f562` (final-object write-ACE allowlist scoped to root-internal objects; pwsh boundary documented: trust the machine's management ACL for the system binary, never send secrets through it). | https://github.com/infogyre/SnipeSpotter/actions/runs/34925785774 |
| `hardware-experiment.yml` dispatch run 19 on `144f562` | `144f562` | FAILED at the collector — **first collector output ever captured**: `collector_stderr.log` = `SecurityError: AuthorizationManager check failed`; LocalSystem's pwsh evaluates the machine execution policy (Restricted on hosted runners; the runner's Bypass lives in the runner user's hive). Fixed by `9ecbb6f` (`-ExecutionPolicy Bypass` scoped to the service's own validated spawn). | https://github.com/infogyre/SnipeSpotter/actions/runs/34926377055 |
| `hardware-experiment.yml` dispatch run 20 on `9ecbb6f` — **LocalSystem observation root-caused and PASSING** | `9ecbb6f` | **SUCCESS on all 6 cells** (windows-2022/-latest × 3 reps): interactive-admin + LocalSystem collection, `validate_report.py` PASS, service stop/deletion/cleanup all confirmed; 6 `hardware-report-*` artifacts + 6 `hardware-diagnostics-*` artifacts uploaded. Run 15's "timeout" was never a collector hang: the service was silently failing at startup (path-policy rejection of the runner layout) and then at collector launch (SYSTEM execution policy), invisible because collector stdout/stderr were nulled and SCM services have no console. Closes the lane-C LocalSystem observation defect for SPOTR-9/24 (AC.10 hosted-workflow evidence incl. SYSTEM actor). | https://github.com/infogyre/SnipeSpotter/actions/runs/34926700265 |
| `release.yml` workflow_dispatch run 10 on `36559ec` (attempts 1–2) | `36559ec5` | FAILED at "Install and assert Rust toolchain": `release.yml` lines 47/65/109 used `^rustc 1\\.98\\.1 ` (double-escaped) in YAML literal blocks, so the grep/-notmatch assertion never matched `rustc 1.98.1` output; all downstream packaging/lifecycle jobs skipped. Fixed by PR #10 (`3777440`); packaging evidence pending re-dispatch on the fix merge. | https://github.com/infogyre/SnipeSpotter/actions/runs/34853457662 |
| `release.yml` workflow_dispatch run 11 on fix merge `5f28061` — **full packaging pipeline PASS, no release published** (dispatch `publish=false`; attest/publish correctly skipped): Verify source PASS (fixed regex), Test workspace PASS, release binaries PASS, "Build MSI from staged binaries" PASS, **"Verify symbols ZIP contains both PDBs" PASS** (release-workflow verifier asserted `spotter_svc.pdb` + `spotter_cli.pdb` inside the produced ZIP before upload), elevated **"Validate MSI lifecycle" PASS** (install/verify/uninstall of the packaged MSI incl. negative installed-PDB inventory checks from `test-msi-lifecycle.ps1`), "Validate direct CLI SCM lifecycle" PASS, Aggregate/closed inventory PASS. Artifacts: `packaged-release`, `msi-lifecycle-logs` (ID 10354221499), `release-inventory`. Closes the previously blocked AC.13–AC.15 native MSI evidence for SPOTR-28. | `5f280612afe1fb0b52b62a64eb7e17ac28839538` | PASS | https://github.com/infogyre/SnipeSpotter/actions/runs/34857173880 |
| `ci.yml` push run 190 on `5f28061` (main post-fix) | `5f280612afe1fb0b52b62a64eb7e17ac28839538` | SUCCESS | https://github.com/infogyre/SnipeSpotter/actions/runs/34857146009 |
